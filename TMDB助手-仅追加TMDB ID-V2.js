// ==UserScript==
// @name         TMDB助手-仅追加TMDB ID - V2
// @namespace    http://tampermonkey.net/
// @version      V4.2
// @description  修复bug + 仅追加TMDB ID + 面板布局优化（防遮挡）
// @author       宝宝
...
(function() {
    ...
    function createUI() {
        ...
        const isAppendIdOnly = String(getVal('tmdb_append_id_only', 'false')) === 'true'; const initAppendIdIdx = isAppendIdOnly ? 1 : 0;
        ...
        GM_addStyle(`
            ...
            #tmdb-ui-container{ position:fixed; bottom:20px; right:20px; width:420px; max-height:calc(100vh - 40px); background:var(--v-bg); color:var(--v-text); border:1px solid var(--v-border); border-radius:var(--v-rm); box-shadow:0 30px 60px rgba(0,0,0,0.6), inset 0 1px 0 rgba(255,255,255,0.2); backdrop-filter:blur(50px) saturate(200%); -webkit-backdrop-filter:blur(50px) saturate(200%); z-index:99998; font-family:var(--v-font); display:flex; flex-direction:column; overflow:hidden; transition: width 0.4s cubic-bezier(0.34, 1.2, 0.64, 1), border-radius 0.4s cubic-bezier(0.34, 1.2, 0.64, 1), background 0.5s ease, padding 0.4s ease; }
            #tmdb-ui-container.collapsed { width: 140px !important; max-height:none; cursor: pointer; border-radius: 30px; background: rgba(20,20,22,0.85); }
            ...
            #tmdb-ui-header { padding:14px 20px 12px; font-weight:700; font-size:16px; display:flex; justify-content:space-between; align-items:center; transition: padding 0.4s ease, font-size 0.5s ease; white-space: nowrap; position: relative; z-index: 10; flex-shrink: 0; }
            ...
            #tmdb-settings-panel { max-height: 0; overflow: hidden; opacity: 0; background: rgba(0,0,0,0.25); box-shadow: inset 0 2px 10px rgba(0,0,0,0.3); transition: all 0.4s cubic-bezier(0.34, 1.2, 0.64, 1); flex-shrink: 0; }
            #tmdb-ui-container.show-settings #tmdb-settings-panel { max-height: 480px; overflow-y: auto; opacity: 1; padding: 12px 16px; border-bottom: 1px solid rgba(255,255,255,0.05); }
            #tmdb-ui-container.collapsed #tmdb-settings-panel { max-height: 0!important; opacity: 0!important; padding: 0 24px!important; }
            .tmdb-ui-body { padding: 0 16px 14px; flex: 1 1 auto; min-height: 0; overflow-y: auto; overflow-x: hidden; opacity: 1; transform: translateY(0) scale(1); transition: opacity 0.4s ease 0.1s, transform 0.5s cubic-bezier(0.34, 1.2, 0.64, 1), padding 0.4s cubic-bezier(0.34, 1.2, 0.64, 1); scrollbar-width: thin; scrollbar-color: rgba(255,255,255,0.15) transparent; }
            .tmdb-ui-body::-webkit-scrollbar { width: 5px; }
            .tmdb-ui-body::-webkit-scrollbar-track { background: transparent; }
            .tmdb-ui-body::-webkit-scrollbar-thumb { background: rgba(255,255,255,0.15); border-radius: 8px; }
            #tmdb-ui-container.collapsed .tmdb-ui-body { max-height: 0; opacity: 0; transform: translateY(-10px) scale(0.98); padding-bottom: 0; pointer-events: none; overflow: hidden; transition: max-height 0.5s cubic-bezier(0.34, 1, 0.64, 1), opacity 0.2s ease, transform 0.4s ease, padding 0.5s cubic-bezier(0.34, 1, 0.64, 1); }
            .tmdb-input-row{display:flex;gap:10px;margin-bottom:8px;} .tmdb-input-group{flex:1; display:flex; flex-direction:column;}
            .tmdb-input-group label{display:block;font-size:11px;font-weight:600;margin-bottom:4px;color:var(--v-text-m); white-space: nowrap;}
            ...
            #tmdb-typography-group .tmdb-segment-indicator, #tmdb-tooltip-toggle-group .tmdb-segment-indicator, #tmdb-panel-width-group .tmdb-segment-indicator, #tmdb-append-id-group .tmdb-segment-indicator { width: calc((100% - 10px) / 2); }
            ...
            .tmdb-segment-btn { flex: 1; background: transparent !important; border: none; color: var(--v-text-m); padding: 6px 0; font-size: 12px; font-weight: 600; cursor: pointer; border-radius: 8px; transition: color 0.3s ease; box-shadow: none !important; transform: none !important; white-space: nowrap; overflow: hidden; }
            .tmdb-segment-btn.active { color: #fff; }
            .tmdb-segment-btn:hover:not(.active) { color: #ddd; background: rgba(255,255,255,0.05) !important; }
            .tmdb-btn-group{display:flex;gap:8px; box-sizing: border-box; margin-bottom: 8px;}
            .tmdb-btn{width:100%;border:1px solid rgba(255,255,255,0.1);padding:9px;border-radius:12px;cursor:pointer;font-weight:700;font-size:13px;font-family:var(--v-font);transition:all 0.3s cubic-bezier(0.34, 1.2, 0.64, 1);backdrop-filter:blur(20px); box-sizing: border-box;}
            ...
            #tmdb-start-btn { background: linear-gradient(135deg, rgba(10,132,255,0.15), rgba(0,201,255,0.15)); border: 1px solid rgba(10,132,255,0.6); box-shadow: inset 0 0 15px rgba(10,132,255,0.2), 0 0 15px rgba(10,132,255,0.4); color: #fff; text-shadow: 0 0 8px rgba(10,132,255,0.8); letter-spacing: 0.5px; padding:10px; font-size: 13px;}
            #tmdb-fix-btn { background: linear-gradient(135deg, rgba(255,159,10,0.15), rgba(255,69,58,0.15)); border: 1px solid rgba(255,159,10,0.6); box-shadow: inset 0 0 15px rgba(255,159,10,0.2), 0 0 15px rgba(255,159,10,0.4); color: #fff; text-shadow: 0 0 8px rgba(255,159,10,0.8); letter-spacing: 0.5px; padding:10px; font-size: 13px;}
            ...
            #tmdb-local-format-btn { background: linear-gradient(135deg, rgba(255,214,10,0.15), rgba(255,159,10,0.15)); border: 1px solid rgba(255,214,10,0.6); box-shadow: inset 0 0 15px rgba(255,214,10,0.2), 0 0 15px rgba(255,214,10,0.4); color: #fff; text-shadow: 0 0 8px rgba(255,214,10,0.8); letter-spacing: 0.5px; width: 100%; padding:10px; font-size:12px; }
            ...
            #tmdb-parent-section { background: rgba(0,0,0,0.15); padding: 10px 12px; border-radius: 14px; box-sizing: border-box; border: 1px solid rgba(255,255,255,0.05); margin-bottom: 12px; width: 100%; transition: all 0.4s cubic-bezier(0.34, 1.2, 0.64, 1); overflow: hidden; }
            .tmdb-parent-header { display: flex; justify-content: space-between; align-items: center; cursor: pointer; user-select: none; margin-bottom: 8px; transition: margin 0.4s; }
            .tmdb-parent-header label { color: #30d158; margin: 0 !important; cursor: pointer; font-size: 12px; }
            .tmdb-parent-toggle-icon { color: var(--v-text-m); font-size: 12px; transition: transform 0.4s cubic-bezier(0.34, 1.2, 0.64, 1); }
            .tmdb-parent-body { max-height: none; opacity: 1; transform: translateY(0); transition: all 0.4s cubic-bezier(0.34, 1.2, 0.64, 1); }
            #tmdb-parent-section.collapsed { padding: 8px 12px; background: rgba(0,0,0,0.25); }
            ...
            #tmdb-log{height:140px;overflow-y:auto;background:rgba(0,0,0,0.15);padding:10px 12px;border-radius:12px;font-size:12px;color:#d1d1d6;line-height:1.5;box-shadow:inset 0 2px 10px rgba(0,0,0,0.2); transition: height 0.4s cubic-bezier(0.34, 1.2, 0.64, 1); flex-shrink: 0;}
            #tmdb-ui-container.height-tall #tmdb-log { height: 240px; }
            #tmdb-ui-container.height-max #tmdb-log { height: 360px; }
        ...
        document.body.insertAdjacentHTML('beforeend', `
            <div id="tmdb-ui-container" class="${isCollapsed ? 'collapsed' : ''} height-${savedHeight} width-${savedWidth}">
                ...
                <div class="tmdb-ui-body">
                    <div class="tmdb-input-row" style="margin-top: 4px;"><button id="tmdb-turbo-btn" class="tmdb-turbo-btn ${isTurbo ? 'active' : ''}" style="width:100%; padding: 9px; border-radius: 12px;" title="点击切换：开启极速模式 (直接API网络通信)"><span class="turbo-text">⚡ 极速API网络连发模式 </span></button></div>
                    <div class="tmdb-input-row" style="margin-bottom: 6px;">
                    ...
                    <div class="tmdb-input-row" style="margin-bottom: 8px;">
                        <div class="tmdb-input-group"><label>重命名模式 / Rename Mode</label><div class="tmdb-segment-control" id="tmdb-append-id-group" style="--idx: ${initAppendIdIdx};"><div class="tmdb-segment-indicator"></div><button class="tmdb-segment-btn ${!isAppendIdOnly ? 'active' : ''}" data-val="0" data-idx="0">完整重命名</button><button class="tmdb-segment-btn ${isAppendIdOnly ? 'active' : ''}" data-val="1" data-idx="1">仅追加ID</button></div></div>
                    </div>
                    <div class="tmdb-btn-group" style="margin-bottom: 10px;"><button id="tmdb-start-btn" class="tmdb-btn" title="按照上方配置，对所有选中的文件进行自动搜刮并重命名">开始刮削 (Auto)</button><button id="tmdb-fix-btn" class="tmdb-btn" title="选中识别有误的项目，强制调出搜索框进行手动搜索">手动修正 (Fix)</button></div>
                    ...
                    <div id="tmdb-parent-section" class="${isParentCollapsed ? 'collapsed' : ''}">
                        ...
                        <div class="tmdb-parent-body">
                            ...
                            <div style="display: flex; gap: 8px; margin-bottom: 0;">
                                <button id="tmdb-select-all-btn" class="tmdb-btn" style="flex: 1; padding: 10px 4px; font-size: 11px; width: auto;" title="一键勾选当前列表的所有文件">☑ 全选</button>
                                <button id="tmdb-series-btn" class="tmdb-btn" style="flex: 1.2; padding: 10px 4px; font-size: 11px; width: auto;" title="提取剧名并自动创建一个系列文件夹，将勾选项移入其中">📁 归档系列</button>
                                <button id="tmdb-local-format-btn" class="tmdb-btn" style="flex: 1.4; padding: 10px 4px; font-size: 11px; width: auto;" title="彻底断网！提取状态栏中已就绪的中英年份信息，本地光速重命名！">📺 批量规范化</button>
                            </div>
                        </div>
                    </div>
        ...
        $$('.tmdb-segment-btn').forEach(btn => {
            btn.onclick = (e) => {
                ...
                else if (parentGroup.id === 'tmdb-append-id-group') GM_setValue('tmdb_append_id_only', targetBtn.getAttribute('data-val') === '1');
    ...
    async function startBatchRename(forceManual = false, forceParent = false) {
        ...
        for (let i = 0; i < targets.length; i++) {
            ...
            try {
                ...
                let appendIdOnly = String(GM_getValue('tmdb_append_id_only', 'false')) === 'true';
                let finalName;
                if (appendIdOnly) {
                    // 仅追加模式：保留原文件名，只在末尾（扩展名之前）追加 .{tmdbid-xxx}
                    let origName = targetItem.domName;
                    // 若已存在旧的 {tmdbid-xxx} / .{tmdbid-xxx} 则先移除，避免重复叠加
                    let nameWithoutExt = info.extension ? origName.slice(0, -info.extension.length) : origName;
                    nameWithoutExt = nameWithoutExt.replace(/\.?\s*\{tmdbid-\d+\}\s*/gi, '').replace(/\.+$/, '').trim();
                    let newName = `${nameWithoutExt}.{tmdbid-${tmdb.id}}`;
                    newName += info.extension;
                    finalName = sanitize(newName);
                } else {
                    ...
                    let newName = `${base} ${sStr ? sStr+' ' : ''}{tmdbid-${tmdb.id}}`.replace(/\s+/g, ' ').replace(' {tmdbid', '{tmdbid').trim(); newName += info.extension; finalName = sanitize(newName);
                }
